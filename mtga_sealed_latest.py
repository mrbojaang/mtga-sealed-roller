...
class SealedTournament:
    ...
    def build_ui(self):
        ...
        tk.Label(setup, text="Best of (Matches):", fg="white", bg=BG).grid(row=0, column=0)
        tk.OptionMenu(setup, self.best_of_group, 1, 3, 5, 7).grid(row=0, column=1)

        tk.Label(setup, text="Rounds:", fg="white", bg=BG).grid(row=0, column=2)
        tk.OptionMenu(setup, self.max_rounds, 1, 2, 3, 4, 5).grid(row=0, column=3)

        tk.Label(setup, text="Best of (Playoffs):", fg="white", bg=BG).grid(row=0, column=4)
        tk.OptionMenu(setup, self.best_of_playoff, 1, 3, 5).grid(row=0, column=5)

        self.entries = []
        for i in range(8):
            e = tk.Entry(setup, width=22)
            e.grid(row=i + 1, column=0, columnspan=6, pady=2)
            self.entries.append(e)

        tk.Button(self.root, text="Start Group Stage", command=self.start_group)\
            .pack(pady=8)

        self.info = tk.Label(self.root, fg="white", bg=BG, font=self.header_font)
        ...
        tk.Button(nav, text="◀ Previous Round", command=self.prev_round).pack(side="left", padx=5)
        tk.Button(nav, text="Undo", command=self.undo).pack(side="left", padx=5)
        tk.Button(nav, text="Next Round ▶", command=self.next_round).pack(side="left", padx=5)
    ...
    def redraw(self):
        for f in (self.match_frame, self.playoff_frame, self.score_frame):
    ...
    def draw_group(self):
        self.info.config(text=f"GROUP STAGE – ROUND {self.current_round}/{self.max_rounds.get()}")
    ...
    def group_match(self, a, b):
        ...
        tk.Label(row, text=f"{a} vs {b}", fg="white", bg=BG, width=22).pack(side="left")

        tk.Button(row, text=a, command=lambda: self.group_result(a, b, a)).pack(side="left")
        tk.Button(row, text="Draw", command=lambda: self.group_result(a, b, None)).pack(side="left")
        tk.Button(row, text=b, command=lambda: self.group_result(a, b, b)).pack(side="left")
    ...
    def draw_group_scoreboard(self):
        ...
        tk.Label(self.score_frame, text="GROUP SCOREBOARD",
                 fg="#f5d76e", bg=BG, font=self.header_font).pack()
    ...
    def playoff_match(self, title, a, b):
        ...
        row = tk.Frame(self.match_frame, bg=BG)
        row.pack(pady=4)
        tk.Label(row, text=f"{a} vs {b}", fg="white", bg=BG).pack(side="left")
